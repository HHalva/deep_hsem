...
parser.add_argument('--nn-dir', default='/home/hermanni/deep_hsem/final_models/',
                    type=str, help='location for trained nn files')
...
parser.add_argument('--out-dir',
                    default='/home/hermanni/deep_hsem/eval_out/',
                    type=str, help='location for evaluation output')
...
def main():
    ...
    nnfile = str(args.dim)+emb_name[0]+".pth.tar"
    if os.path.isfile(os.path.join(args.nn_dir, nnfile)):
        print("=> loading checkpoint '{}'".format(nnfile))
        checkpoint = torch.load(os.path.join(args.nn_dir, nnfile))
        model.load_state_dict(checkpoint['state_dict'])
        print("=> loaded checkpoint '{}'".format(nnfile,
                                                 checkpoint['epoch']))
    else:
        print("=> no checkpoint found at '{}'".format(nnfile))
        sys.exit()
    ...
    cond_accs_dict = {'top-1': [], 'top-2': [], 'top-5': [],
                      'top-10': [], 'top-20': []}
    num_leaves_dict = {'top-1': [], 'top-2': [], 'top-5': [],
                       'top-10': [], 'top-20': []}
    hop_dists_dict = {'top-1': [], 'top-2': [], 'top-5': [],
                      'top-10': [], 'top-20': []}
    target_dists_dict = {'top-1': [], 'top-2': [], 'top-5': [],
                         'top-10': [], 'top-20': []}
    ...
    assert args.subgraph in ['all', 'mammal', 'vehicle', 'food', 'furniture',
    'bird', 'musical_instrument', 'geological_formation']
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(eval_loader):
            ...
            cond_accs, num_leaves_under, dist_to_target = wn_eval(output, emb_wgts, target_ss,
                                                                  idx_to_class_map,
            ...
            num_leaves_dict['top-1'].extend(num_leaves_under[0])
            num_leaves_dict['top-2'].extend(num_leaves_under[1])
            num_leaves_dict['top-5'].extend(num_leaves_under[2])
            num_leaves_dict['top-10'].extend(num_leaves_under[3])
            num_leaves_dict['top-20'].extend(num_leaves_under[4])

            target_dists_dict['top-1'].extend(dist_to_target[0])
            target_dists_dict['top-2'].extend(dist_to_target[1])
            target_dists_dict['top-5'].extend(dist_to_target[2])
            target_dists_dict['top-10'].extend(dist_to_target[3])
            target_dists_dict['top-20'].extend(dist_to_target[4])
        ...
        out_prefix = os.path.join(args.out_dir, fprefix)
        with open(out_prefix+"_cond_accs.txt", "wb") as fp:
            pickle.dump(cond_accs_dict, fp)

        with open(out_prefix+"_num_leaves.txt", "wb") as fp:
            pickle.dump(num_leaves_dict, fp)

        with open(out_prefix+"_hop_dists.txt", "wb") as fp:
            pickle.dump(hop_dists_dict, fp)

        with open(out_prefix+"_target_dists.txt", "wb") as fp:
            pickle.dump(target_dists_dict, fp)
...
def wn_eval(output, all_embs, target_synset, idx_to_class_dictionary,
    ...
    with torch.no_grad():
        ...
        preds_tree_evals = [tree_eval(target_synset, p)
                            for p in preds_ss]
        cond_accs, num_leaves_under, dists_to_target = list(zip(*preds_tree_evals))
        cond_accs_list, n_leaves_under_list, dists_2_target_list = [], [], []
        for k in topk:
...
def tree_eval(target_ss, prediction_ss):
    hyper_fun = lambda s: s.hypernyms()
    target_hypers = [target_ss]+list(target_ss.closure(hyper_fun))
    if prediction_ss in target_hypers:
        cond_acc = 1
    else:
        cond_acc = 0
    dist_to_target = target_ss.shortest_path_distance(prediction_ss)
    # calculate number of leaves under prediction
    n_leaves_under = 0
    all_leaves = False
    prediction_hypos = prediction_ss.hyponyms()
    if len(prediction_hypos) == 0:
        all_leaves = True
    while not all_leaves:
        ph = [p.hyponyms() for p in prediction_hypos]
        is_leaf_hypo = [len(p) == 0 for p in ph]
        n_leaves_under += sum(is_leaf_hypo)
        all_leaves = all(is_leaf_hypo)
        prediction_hypos = list(itertools.chain(*ph))
    return cond_acc, n_leaves_under, dist_to_target
