...
os.environ["CUDA_VISIBLE_DEVICES"] = "0, 1, 2, 4, 4, 5, 6, 7"
...
from lorentz_loss import EuclideanEmbeddingXELoss
...
parser.add_argument('--emb_dir',
                    default='/home/hermanni/final_embeddings/', type=str, help='location for embedding files')
...
def main():
    ...
    global imgnet_euc_wgt, imgnet_labels
    ...
    print('EMBEDDING TYPE: Euclidean')
...
def main_worker(gpu, ngpus_per_node, args):
    global best_prec1, imgnet_euc_wgt
    args.gpu = gpu
    ...
    #change model to project into euclidean embedding space
    model = EuclidEmbVGG(orig_vgg, args.n_emb_dims, args.unfreeze)
    ...
    criterion = EuclideanEmbeddingXELoss()
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output & loss
        output = model(images)
        loss = criterion(output, target, imgnet_euc_wgt)

        # measure accuracy and record loss
        prec1, prec5 = accuracy(output, imgnet_euc_wgt, target, topk=(1, 5))
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # measure accuracy and record loss
            prec1, prec5 = accuracy(output, imgnet_euc_wgt,
                                    target, topk=(1, 5))
            losses.update(loss.item(), images.size(0))
            top1.update(prec1.item(), images.size(0))
            top5.update(prec5.item(), images.size(0))
...
def prediction(output, all_embs, knn=1):
    """Predicts the nearest class based on euclidean distance"""
    with torch.no_grad():
        ...
        dists_to_all = F.pairwise_distance(expand_output, expand_all_embs, keepdim=True)
...
def save_checkpoint(state, is_best, is_finetune, filename):
    if args.unfreeze:
        torch.save(state, 'finetune_'+filename)
        if is_best:
            shutil.copyfile('finetune_'+filename, 'best_finet_'+filename)
    else:
