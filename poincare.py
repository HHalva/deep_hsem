...
class Distance(Function):
    ...
    @staticmethod
    def backward(ctx, g):
        pmanif = PoincareManifold()
        u, v, squnorm, sqvnorm, sqdist = ctx.saved_tensors
        g = g.unsqueeze(-1)
        gu = Distance.grad(u, v, squnorm, sqvnorm, sqdist, ctx.eps)
        gv = Distance.grad(v, u, sqvnorm, squnorm, sqdist, ctx.eps)
        euc_g_u = g.expand_as(gu) * gu
        euc_g_v = g.expand_as(gv) * gv
        return euc_g_u, euc_g_v, None
        #return pmanif.rgrad(u, euc_g_u), pmanif.rgrad(v, euc_g_v), None
