...
from lorentz_loss import LorentzXEntropyLoss
...
parser.add_argument('--lr-decay-interval', default=30, type=float,
                    metavar='N', help='decays lr every N epcoh')
...
def main():
    global args, best_prec1, lorentz_emb
    global imgnet_lorentz_wgt, imgnet_labels
    ...
    print('EMBEDDING TYPE:', lorentz_emb['manifold'])
    n_emb_dims = lorentz_emb['embeddings'].shape[1]-1
    args.n_emb_dims = n_emb_dims
    print('NUM OF DIMENSIONS:', n_emb_dims)

    #change labels from synset names into imagenet format
    synset_list = [wn.synset(i) for i in lorentz_emb['objects']]
    offset_list = [wn.ss2of(j) for j in synset_list]
    lorentz_emb['objects'] = ['n'+i.split('-')[0] for i in offset_list]
...
def main_worker(gpu, ngpus_per_node, args):
    global best_prec1, imgnet_lorentz_wgt
    args.gpu = gpu
    ...
    #change model to project into Lorentzian embedding space
    model = LorentzEmbVGG(orig_vgg, args.n_emb_dims, args.unfreeze)
    ...
    imgnet2emb_idx = [lorentz_emb['objects'].index(i)
                      for i in imgnet_labels]
    imgnet_lorentz_wgt = lorentz_emb['embeddings'][imgnet2emb_idx]
    imgnet_lorentz_wgt = imgnet_lorentz_wgt.float().cuda(non_blocking=True)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output & loss
        output = model(images)
        loss = criterion(output, target, imgnet_lorentz_wgt)

        # measure accuracy and record loss
        prec1, prec5 = accuracy(output, imgnet_lorentz_wgt, target, topk=(1, 5))
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # measure accuracy and record loss
            prec1, prec5 = accuracy(output, imgnet_lorentz_wgt,
                                    target, topk=(1, 5))
            losses.update(loss.item(), images.size(0))
            top1.update(prec1.item(), images.size(0))
            top5.update(prec5.item(), images.size(0))
...
class LorentzEmbVGG(nn.Module):
    def __init__(self, vgg_model, n_emb_dims, unfreeze=False):
        super(LorentzEmbVGG, self).__init__()
    ...
    def forward(self, x):
        ...
        f = f.view(f.size(0), -1)
        y = self.classifier(f)
        y0 = torch.sqrt(torch.sum((y)**2, dim=1, keepdim=True)+1)
        return torch.cat((y0, y), 1)

def prediction(output, all_embs, knn=1):
    """Predicts the nearest class based on Lorentzian distance"""
    with torch.no_grad():
        batch_size = output.size(0)
        n_emb_dims = output.size(1)
        n_classes = all_embs.size(0)
        expand_output = output.repeat(1, n_classes).view(-1, n_emb_dims)
        expand_all_embs = all_embs.repeat(batch_size, 1).cuda(args.gpu, non_blocking=True)
        dists_to_all = LorentzManifold().distance(expand_output, expand_all_embs)
