...
def main_worker(gpu, ngpus_per_node, args):
    global best_prec1, imgnet_poinc_wgt
    args.gpu = gpu
    ...
    import csv
    with open('mammals.csv', 'w', newline='') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        wr.writerow(poinc_emb['objects'])

    pdb.set_trace()

    imgnet_labels = train_dataset.classes
    imgnet2emb_idx = [poinc_emb['objects'].index(i)
                      for i in imgnet_labels]
    imgnet_poinc_wgt = poinc_emb['embeddings'][imgnet2emb_idx]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, args.emb_name+'mammal_checkp.pth.tar')
...
def prediction(output, all_embs, knn=1):
    ...
    with torch.no_grad():
        ...
        dists_to_all = PoincareManifold().distance(expand_output, expand_all_embs)
